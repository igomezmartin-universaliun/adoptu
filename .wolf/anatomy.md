...
- `versions.tf` — OpenTofu/AWS provider version pins, backend notes (local by default) (~150 tok)
...
- `data.tf` — read-only lookups: Route53 zone, ACM certs (regional + us-east-1), ECR repo, default VPC, default dual-stack subnets, DB subnet group, rds-monitoring-role (~350 tok)
...
- `security_groups.tf` — ALB SG (public 443/80), ECS task SG (from ALB only), RDS SG (from ECS task only) (~300 tok)
...
- `ecs.tf` — new `adoptu` ECS cluster/service/task def, Fargate, deployed into the IPv6-only subnets; container_port fixed to 8080 (live task def had 80, app actually listens on 8080) (~450 tok)
- `alb.tf` — public ALB, `dualstack-without-public-ipv4` (IPv6-reachable, no public IPv4 ever allocated), HTTPS/HTTP listeners, IPv6 target group (~250 tok)
- `s3.tf` — `adoptu-static-images`/`adoptu-dynamic-images` buckets, public access block, CloudFront-OAC-only bucket policies (~250 tok)
- `cloudfront.tf` — 3 distributions: static/dynamic image origins (S3+OAC), app origin (ALB, replacing the live hardcoded-task-IP origin) (~450 tok)
- `route53.tf` — A/AAAA alias records for apex/www/api -> app CloudFront, static/dynamic -> their CloudFront dists. Other ~20 zone records (mail, SES verification, NS/SOA) deliberately untouched (~300 tok)
- `ses.tf` — comment-only; SES domain identity already verified live, intentionally not managed here (~80 tok)
- `outputs.tf` — alb_dns_name, rds_endpoint, cloudfront domains, ecs subnet ids (~80 tok)
- `terraform.tfvars.example` — sample values matching the live account (~60 tok)
- `README.md` — full import plan (this is a brownfield account, not greenfield — S3/RDS/CloudFront/IAM-role names already exist live and must be `tofu import`ed, not created fresh), 4-step migration/cutover plan, list of live-config findings/fixes, old-resource decommission commands (~1800 tok)
