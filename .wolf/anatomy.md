...
- `cloudfront.tf` — 3 distributions: static/dynamic image origins (S3+OAC), app origin is `ecs.<domain>` directly (custom origin, `http-only`/8080 — verified to match the live distribution's actual config exactly, NOT an ALB) (~450 tok)
- `route53.tf` — `ecs.<domain>` AAAA record (origin for the app CloudFront dist, `lifecycle.ignore_changes` on `records` since it tracks whichever task is currently running and is updated operationally, not by Terraform) + A/AAAA alias records for apex/www/api -> app CloudFront, static/dynamic -> their CloudFront dists. Other ~20 zone records (mail, SES verification, NS/SOA) deliberately untouched (~350 tok)
- `ses.tf` — comment-only; SES domain identity already verified live, intentionally not managed here (~80 tok)
- `outputs.tf` — ecs_task_origin_record (the DNS name to re-point after each deploy), rds_endpoint, cloudfront domains, ecs subnet ids (~80 tok)
- `terraform.tfvars.example` — sample values matching the live account (~60 tok)
- `README.md` — full import plan (this is a brownfield account, not greenfield — S3/RDS/CloudFront/IAM-role names already exist live and must be `tofu import`ed, not created fresh), 4-step migration/cutover plan (includes the manual `ecs.<domain>` re-point step since there's no LB), list of live-config findings/fixes, old-resource decommission commands (~1900 tok)
