...
- **`gradle/wrapper/gradle-wrapper.jar` is untracked by git** (only `gradle-wrapper.properties` is committed) — fresh worktrees won't have it and `./gradlew` fails with "Unable to access jarfile". Copy it from the main checkout (`cp /home/laf/Proyektes/adoptu/gradle/wrapper/gradle-wrapper.jar gradle/wrapper/`) before running gradle in a new worktree.
