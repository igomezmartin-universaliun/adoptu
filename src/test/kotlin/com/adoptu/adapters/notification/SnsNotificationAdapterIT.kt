...
class SnsNotificationAdapterIT {

    private var localstackContainer: LocalStackContainer? = null
    private var testTopicArn: String? = null
    ...
    fun startLocalStack() {
        ...
        val endpointOverride = localstackContainer!!.getEndpointOverride(LocalStackContainer.Service.SNS)
        ...
        val topicResponse = snsClient.createTopic(CreateTopicRequest.builder().name("test-topic").build())
        testTopicArn = topicResponse.topicArn()
    ...
    private fun createConfig(): MapApplicationConfig {
        ...
        return MapApplicationConfig(
            "sns.region" to ls.region,
            "sns.access_key_id" to "test",
            "sns.secret_access_key" to "test",
            "sns.topic_arn" to testTopicArn!!,
            "sns.endpoint" to ls.getEndpointOverride(LocalStackContainer.Service.SNS).toString()
        )
    }

    @Test
    fun `sendEmail returns true when configured with LocalStack`() = runBlocking {
    ...
    @Test
    fun `sendEmail sends correct message to SNS topic`() = runBlocking {
        val config = createConfig()
        val adapter = SnsNotificationAdapter(config)
