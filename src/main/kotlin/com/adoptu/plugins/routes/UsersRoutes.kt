...
fun Route.usersRoutes() {
    ...
    route("/api/users") {
        ...
        put("/photographer-settings") {
            ...
            try {
                val photographer = userService.updatePhotographerSettings(session.userId, body)
                    ?: return@put call.respondError("User not found", 404)
        ...
        post("/photography-requests/multiple") {
            val session = call.sessions.get<SessionUser>()
                ?: return@post call.respondError("Unauthorized", 401)
