...
class UserShelterRoutesE2ETest {
    ...
    private fun createShelterInDb(userId: Int, country: String = "USA", state: String? = "CA", city: String = "LA") {
        transaction {
    ...
    fun `GET user-shelters returns matching shelters`() {
        createShelterInDb(3, country = "USA", state = "CA", city = "LA")

        testApplication {
            setupApp()
            val response = client.get("/api/user-shelters?country=USA&state=CA&city=LA")
            assertEquals(HttpStatusCode.OK, response.status)
    ...
    fun `GET user-shelters returns empty list for non-matching country`() {
        createShelterInDb(3, country = "USA")
