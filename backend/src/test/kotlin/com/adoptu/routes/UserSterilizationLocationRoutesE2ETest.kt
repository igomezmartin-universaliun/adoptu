...
class UserSterilizationLocationRoutesE2ETest {
    ...
    private fun createLocationInDb(userId: Int, country: String = "USA", state: String? = "CA", city: String = "LA") {
        transaction {
    ...
    fun `GET user-sterilization-locations returns matching locations`() {
        createLocationInDb(3, country = "USA", state = "CA", city = "LA")

        testApplication {
            setupApp()
            val response = client.get("/api/user-sterilization-locations?country=USA&state=CA&city=LA")
            assertEquals(HttpStatusCode.OK, response.status)
    ...
    fun `GET user-sterilization-locations returns empty list for non-matching country`() {
        createLocationInDb(3, country = "USA")
